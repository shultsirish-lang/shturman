...
export default {
  fetch: withSupabase({ auth: ["publishable", "secret"] }, async (req, ctx) => {
    ...
    const catalogColumns = "id,provider,code,title,specialty,topics,keywords,source_name,source_version,source_pages,green_clinic_code,price_rub,method,biomaterial,prep,material,duration,updated_at";
    ...
    if (req.method === "POST" && path === "/admin/lab-catalog/import") {
      ...
      const rows = payload.items.map((item: Record<string, unknown>) => ({
        ...
        price_rub: item.price_rub ?? null, method: item.method ?? null, biomaterial: item.biomaterial ?? null,
        prep: item.prep ?? null, material: item.material ?? null, duration: item.duration ?? null, updated_at: new Date().toISOString(),
      }));
